...
def translate_text(text, dest_lang='my'):
    """Google Translate API ကိုသုံးပြီး ဘာသာပြန်မယ် (Backup အနေနဲ့)"""
    try:
...
def index():
    ...
    <body>
        <div class="container">
            ...
            <div id="manual-translation" style="display:none; margin-top: 20px; padding: 15px; background: #f0f0f0; border-radius: 8px;">
                <label for="translation-text" style="font-weight: bold;">📝 ဘာသာပြန်စာသားထည့်ပါ (Gemini ကရတဲ့စာသား):</label>
                <textarea id="translation-text" rows="5" style="width:100%; padding:10px; margin-top:5px; border:2px solid #ddd; border-radius:8px; font-family: 'Pyidaungsu', Arial;"></textarea>
                <p style="color: #666; font-size: 14px; margin-top:5px;">Gemini က ရလာတဲ့ စာသားကို ဒီမှာ Paste လုပ်ပါ (Ctrl+V)</p>
            </div>
        ...
        <script>
            ...
            function selectOption(opt) {
                ...
                if (opt === 'video') {
                    ...
                    document.getElementById('manual-translation').style.display = 'block';
                } else if (opt === 'audio') {
                    document.getElementById('video-options').style.display = 'none';
                    document.getElementById('manual-translation').style.display = 'block';
                } else {
                    document.getElementById('video-options').style.display = 'none';
                    document.getElementById('manual-translation').style.display = 'none';
                }
            }
            ...
            async function processVideo() {
                ...
                const manualText = document.getElementById('translation-text')?.value || '';
...
def process():
    ...
    manual_translation = data.get('manual_translation', '')
    ...
    try:
        if option == 'original_video':
            ...
            # လက်နဲ့ထည့်ထားတဲ့ ဘာသာပြန်စာသားရှိရင် အဲဒါကိုသုံးမယ်
            translated = manual_translation if manual_translation else translate_text(text, target_lang)
